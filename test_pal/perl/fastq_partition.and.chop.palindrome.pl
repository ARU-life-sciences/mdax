...
if ( $filenameA=~ /.gz$/ ) {
    open (IN, "gzip -dc $filenameA |") or die "oops!\n" ;
}
else {
...
open (my $gzip_in, "| gzip -c > $filenameA.include.fq.gz") or die "error starting gzip $!";
open (my $gzip_ex, "| gzip -c > $filenameA.exclude.fq.gz") or die "error starting gzip $!";
